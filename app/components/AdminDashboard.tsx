...
const orders = [
  { id: 'AP-7K2M-XP4A', name: 'Marina Lima', email: 'marina@email.com', value: 'R$ 67,90', time: '12:48', status: 'Ativação em andamento', tone: 'amber' },
  { id: 'AP-4Q8N-CL7R', name: 'Rafael Souza', email: 'rafael@email.com', value: 'R$ 67,90', time: '12:21', status: 'Acesso entregue', tone: 'mint' },
          ...
          <div className="p-4 sm:p-7">
            {activeView === 'Visão geral' ? (
              <>
                <section className="grid gap-4 sm:grid-cols-2 xl:grid-cols-4">
                  {[
                    ...
                    ['Tempo médio de entrega', '18 min', '-4 min', Activity, 'blue'],
                ...
                <section className="mt-5 grid gap-5 xl:grid-cols-[1fr_340px]">
                  <div className="rounded-[22px] border border-[#dfe4ec] bg-white p-5 sm:p-6">
                    <div className="flex items-center justify-between"><div><h2 className="font-extrabold tracking-[-.025em]">Receita nos últimos 7 dias</h2><p className="mt-1 text-xs text-[#838ca0]">Dados ilustrativos do painel demonstrativo</p></div><span className="rounded-xl bg-[#f3f5f9] px-3 py-2 text-xs font-bold text-[#66718a]">7 dias</span></div>
                  ...
                  <div className="rounded-[22px] border border-[#dfe4ec] bg-white p-5 sm:p-6">
                    ...
                    <p className="mt-5 rounded-xl bg-[#f6f7fa] p-3 text-[11px] leading-5 text-[#727c92]">Ajuste manual ativo apenas nesta demonstração. Em produção, as mudanças ficam registradas no histórico de atividades.</p>
                  </div>
                </section>

                <section className="mt-5 rounded-[22px] border border-[#dfe4ec] bg-white">
                  <div className="flex items-center justify-between border-b border-[#e5e9f0] px-5 py-5 sm:px-6"><div><h2 className="font-extrabold tracking-[-.025em]">Pedidos recentes</h2><p className="mt-1 text-xs text-[#838ca0]">Acompanhe pagamentos e entregas</p></div><button onClick={() => setActiveView('Pedidos')} className="text-xs font-extrabold text-[#5d50cb]">Ver todos</button></div>
              ...
              <section className="order-workspace">
                ...
                <div className="order-chat-panel"><div className="flex items-center justify-between border-b border-[#e4e8ef] p-4"><div><p className="font-extrabold">{selectedOrder.name}</p><p className="text-[10px] text-[#7d879c]">Pedido {selectedOrder.id}</p></div><button className="rounded-xl border border-[#dce2eb] px-3 py-2 text-[10px] font-extrabold">ASSUMIR ATENDIMENTO</button></div><div className="flex-1 space-y-4 overflow-y-auto p-5"><div className="chat-bubble assistant-bubble">Olá! Seu pagamento foi confirmado e a ativação está em andamento.</div><div className="flex justify-end"><div className="chat-bubble customer-bubble">Perfeito, obrigado! Vou aguardar por aqui.</div></div><div className="system-message mx-auto">Pedido e conversa vinculados</div></div><div className="border-t border-[#e4e8ef] p-3"><div className="flex items-center gap-2 rounded-xl border border-[#dce2eb] bg-[#f7f8fb] p-2"><input className="min-w-0 flex-1 bg-transparent px-2 text-xs outline-none" placeholder="Digite uma resposta..." /><button className="grid h-9 w-9 place-items-center rounded-lg bg-[#192444] text-white"><Send size={15} /></button></div></div></div>
                <div className="order-detail-panel"><div className="border-b border-[#e4e8ef] p-4"><h2 className="font-extrabold">Detalhes</h2><p className="mt-1 text-xs text-[#858ea2]">Processamento da entrega</p></div><div className="space-y-5 p-4"><div className="rounded-xl bg-[#f6f7fa] p-3 text-xs"><div className="flex justify-between"><span className="text-[#7c8599]">Produto</span><strong>Gemini Pro</strong></div><div className="mt-3 flex justify-between"><span className="text-[#7c8599]">Valor</span><strong>{selectedOrder.value}</strong></div><div className="mt-3 flex justify-between"><span className="text-[#7c8599]">Status</span><span className={`status-pill ${selectedOrder.tone}`}>{selectedOrder.status}</span></div></div><div><label htmlFor="delivery-url" className="text-[10px] font-extrabold uppercase tracking-[.1em] text-[#68738d]">Link de acesso</label><input id="delivery-url" value={deliveryUrl} onChange={(event) => { setDeliveryUrl(event.target.value); setDelivered(false); }} className="mt-2 h-11 w-full rounded-xl border border-[#dce2eb] px-3 text-xs outline-none focus:border-[#6557d8]" placeholder="https://..." /><button onClick={deliver} className="mt-3 flex min-h-11 w-full items-center justify-center gap-2 rounded-xl bg-[#192444] text-xs font-extrabold text-white"><Send size={14} /> Enviar para o cliente</button>{delivered && <p className="mt-3 rounded-xl bg-[#e6f9f5] p-3 text-xs font-bold text-[#147362]">Link validado e entrega demonstrativa registrada.</p>}</div><div className="border-t border-[#e5e9f0] pt-4 text-[10px] leading-5 text-[#858da0]">A entrega em produção exige confirmação, impede duplicidade e gera um registro de auditoria com operador, horário e identificadores do pedido e da conversa.</div></div></div>
              </section>
            ) : (
